...
if (!process.env.API_KEY) {
  // In a real app, you might not want to throw, but for deployment, this is a clear indicator.
  console.warn("API_KEY environment variable not set. The AI service will not work.");
}
...
// --- API Endpoints ---
// All your API logic now lives here, prefixed with /api/
app.post('/api/analyze-platform', async (req, res) => {
  ...
  // Fallback if API_KEY is not set
  if (!process.env.API_KEY) {
...
app.post('/api/video-details', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      downloadUrl: metadata.url, // This might be a direct link depending on the extractor
      isValid: true,
    });
  } catch (error) {
    console.error('[Server] yt-dlp Error:', error);
